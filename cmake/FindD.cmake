...
## Find D compiler from user or PATH
find_program(DC NAMES dmd ldc)
set(DC "${DC}" CACHE STRING "Manually select compiler")

if (DC)
    get_filename_component(dc_name ${DC} NAME)

    ## Configure for the found compiler
    if (dc_name STREQUAL "dmd")
        set(D_IS_MARS true)
        set(D_IS_DMD true)
        set(D_COMPILER_NAME "Digital Mars D Compiler")
        set(dc_version_switch "")
    elseif (dc_name STREQUAL "ldc")
        set(D_IS_LLVM true)
        set(D_IS_LDC true)
        set(D_COMPILER_NAME "LLVM-based D Compiler")
        set(dc_version_switch "--version")

        if (${CMAKE_SYSTEM_PROCESSOR} MATCHES "^i.86$")
            set(LDC_DEFAULT_CPU "i686")
        elseif(${CMAKE_SYSTEM_PROCESSOR} MATCHES "^(amd|x86[_-])64$")
            set(LDC_DEFAULT_CPU "x86-64")
        else (${CMAKE_SYSTEM_PROCESSOR} MATCHES "^i.86$")
            set(LDC_DEFAULT_CPU "")
        endif(${CMAKE_SYSTEM_PROCESSOR} MATCHES "^i.86$")
        set(LDC_CPU ${LDC_DEFAULT_CPU} CACHE STRING "Set the CPU for LLVM to generate for")
    else (dc_name STREQUAL "dmd")
        message(FATAL_ERROR "Unknown compiler ${DC}")
    endif (dc_name STREQUAL "dmd")

    ## Figure out versions.
    exec_program(${DC} ARGS "${dc_version_switch}" OUTPUT_VARIABLE d_output)
    string(REGEX MATCH "v([0-9])\\.([0-9]+)" matches "${d_output}")

    if (NOT matches)
        message(FATAL_ERROR "${DC} compiler output doesn't match")
    endif (NOT matches)

    set(D_VERSION "${CMAKE_MATCH_1}")
    set(D_FRONTEND ${CMAKE_MATCH_2})
else (DC)
    message(FATAL_ERROR "No D compiler found")
endif (DC)
