...
struct Signal( Args... )
{
    ...
    {
        volatile if ((!atomicAdd(m_running, 1)) && m_dirty)
        {
            m_shadowDgs.length = m_dgs.length;
            m_shadowDgs[0..$] = m_dgs[0..$];
            m_shadowFns.length = m_fns.length;
            m_shadowFns[0..$] = m_fns[0..$];
            m_dirty = false;
        }
        scope(exit) volatile atomicAdd(m_running, -1);

        for( size_t i = 0; i < m_shadowDgs.length; ++i )
            m_shadowDgs[i]( args );

        for( size_t i = 0; i < m_shadowFns.length; ++i )
            m_shadowFns[i]( args );
    }
